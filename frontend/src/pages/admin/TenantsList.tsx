...
export function TenantsList() {
    ...
    async function fetchTenants() {
        try {
            // TODO: Implement backend endpoint for listing tenants (admin only)
            // For now, we might need to mock or assume endpoint exists.
            // Let's assume we'll create GET /api/admin/tenants
            const data = await api.get('/admin/tenants');
            setTenants(data);
        } catch (error) {
            console.error("Failed to fetch tenants", error);
            // Mock data for UI development if backend fails (or isn't ready)
            setTenants([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleCreateTenant = async (e: React.FormEvent) => {
        ...
        try {
            await api.post('/admin/tenants', {
                ...newTenant,
                cnpj: newTenant.cnpj.replace(/\D/g, '') // Enviar apenas números
            });
            ...
            // @ts-ignore
            const errorMessage = error.response?.data?.detail || "Erro ao criar escritório.";
            alert(errorMessage);
        }
    };
    ...
    const handleUpdateTenant = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert("Erro ao atualizar escritório.");
        }
    };

    const handleDeleteTenant = async (tenant: any) => {
        if (!confirm(`CUIDADO: Isso excluirá permanentemente o escritório "${tenant.nome}" e TODOS os seus dados. Deseja continuar?`)) return;
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500" onClick={() => setActiveMenu(null)}>
            ...
            <div className="bg-end-card border border-end-border rounded-xl">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-white/5 border-b border-white/10">
                            ...
                            <th className="p-4 text-xs font-bold text-end-text-sec uppercase">Plano</th>
                            <th className="p-4 text-xs font-bold text-end-text-sec uppercase">Data Cadastro</th>
                                ...
                                <td className="p-4 text-right relative">
                                    ...
                                    {activeMenu === tenant.id && (
                                        <div className="absolute right-4 top-12 z-20 bg-end-card border border-end-border rounded-lg shadow-2xl py-2 w-48 animate-in zoom-in-95 duration-100">
                                            ...
                                            <button
                                                onClick={() => window.location.href = `/admin/users?tenant_id=${tenant.id}`}
                                                className="w-full px-4 py-2 text-sm text-white hover:bg-white/10 flex items-center gap-2"
                                            >
                        ...
                        <form onSubmit={handleCreateTenant} className="space-y-4">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={e => setNewTenant({ ...newTenant, cnpj: formatCNPJ(e.target.value) })}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-end-text-sec uppercase mb-1">Plano</label>
                                <select
                        ...
                        <form onSubmit={handleUpdateTenant} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={e => setEditingTenant({ ...editingTenant, nome: e.target.value })}
                                    className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white outline-none focus:border-end-accent"
                                    required
                                />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={e => setEditingTenant({ ...editingTenant, cnpj: formatCNPJ(e.target.value) })}
                                    className="w-full bg-black/20 border border-white/10 rounded-lg p-3 text-white outline-none focus:border-end-accent"
                                    required
                                />
                            </div>
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button type="submit" className="flex-1 px-4 py-2 bg-end-accent text-black font-bold rounded-lg hover:bg-end-accent/90">Salvar Alterações</button>
                            </div>
                        </form>
                    </div>
                </div>
